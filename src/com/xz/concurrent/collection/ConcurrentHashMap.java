...
/**
 * JDK1.7 之前使用锁分段保证线程安全 JDK 1.8使用CAS和synchronized来保证线程安全
...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
    ...
    /**
     * 默认容量为16 加载因子 0.75 concurrencyLevel 16
     * concurrencyLevel:确定sizeCtl的大小，并不表示并发数，有多少桶就允许多少并发数
     */
    public ConcurrentHashMap() {
    }

    /**
     * 创建指定大小的ConcurrentHashMap 加载银子0.75  concurrencyLevel 0.75
     */
    ...
    /**
     * 用于创建一个带有指定初始容量、加载因子和默认 concurrencyLevel (1) 的新的空映射。
     */
    ...
    /**
     * 用于创建一个带有指定初始容量、加载因子和并发级别的新的空映射
     */
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K, V>[] tab = table; ; ) {
            ...
            //数组不为空，key的下表对应的位置为null，创建新的节点加入
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {
                if (casTabAt(tab, i, null, new Node<K, V>(hash, key, value, null))) {
                    break;
                }
            }
            //检测到桶节点是forwardingNode类型，协助扩容
            else if ((fh = f.hash) == MOVED) {
                tab = helpTransfer(tab, f);
            }
            //桶结点是普通节点，锁住该桶头节点比并试图在该链表的尾部添加新的节点
            else {
                V oldVal = null;
                synchronized (f) {
                    if (tabAt(tab, i) == f) {
                ...
                //binCount !=0 表明向链表或者红黑树中添加或修改一个节点成功。
                //binCount == 0 表明put操作将一个新节点添加成为某个桶的新节点
                if (binCount != 0) {
                    //链表深度>8 转换为红黑树
                    if (binCount >= TREEIFY_THRESHOLD) {
                        treeifyBin(tab, i);
                    }
                    //oldVal ！= null 表示此次操作是修改操作 直接返回旧值即可
                    if (oldVal != null) {
                        return oldVal;
                    }
                    break;
    ...
    public boolean equals(Object o) {
        if (o != this) {
            ...
            for (Map.Entry<?, ?> e : m.entrySet()) {
                Object mk, mv, v;
                if ((mk = e.getKey()) == null || (mv = e.getValue()) == null || (v = get(mk)) == null || (mv != v && !mv
                        .equals(v))) {
                    return false;
                }
            }
    ...
    private final Node<K, V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            //sizeCtl<0，表示有线程在进行相同的操作
            if ((sc = sizeCtl) < 0) {
                Thread.yield();
            }
            //未有线程进行初始化，有该线程进行初始化操作
            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                try {
                    //再次确定数组是否被初始化，确保唯有其他线程进行初始化
